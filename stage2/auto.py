...
class Autoset:
    ...
    def auto(self):
        ...
        if self.check_devices():
            ...
            self.master.set_message("Can't start, check devices")
    ...
    def recognize(self):
        ...
        Thread(target=self.show_video).start()

        self.rbt.go_to(z=self.z_scan)
        ...
        for region, loc in enumerate(locs):
            ...
            self.visualize_f.set_viable(len(self.total))
            sleep(1)
    ...
    def seed(self):
        ...
        for i in range(len(self.to_seed)):
            ...
            self.visualize_f.set_sown(i+1)
            self.visualize_f.set_progress(f"{i+1 // total}%")
...
# ------------------------------AUXILIAR-METHODS---------------
    ...
    def put_seed(self, pos):
        ...
        self.rbt.go_to(x=pos[0], y=pos[1], z=self.planting_depth)
        self.rbt.wait_to(x=pos[0], y=pos[1], z=self.planting_depth)
