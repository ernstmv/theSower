...
void limits_init()
{
  // printPgmString(PSTR("int_en\n\r"));
  LIMIT_DDR &= ~(LIMIT_MASK2); // Set as input pins

  #ifdef DISABLE_LIMIT_PIN_PULL_UP
    LIMIT_PORT &= ~(LIMIT_MASK); // Normal low operation. Requires external pull-down.
  #else
    LIMIT_PORT |= (LIMIT_MASK);  // Enable internal pull-up resistors. Normal high operation.
    LIMIT_PORT &= ~(1<<SENSOR_SEED);
  #endif

  if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
    LIMIT_PCMSK |= LIMIT_MASK2; // Enable specific pins of the Pin Change Interrupt
...
void limits_disable()
{
  //printPgmString(PSTR("int_de\n\r"));
  LIMIT_PCMSK &= ~LIMIT_MASK2;  // Disable specific pins of the Pin Change Interrupt
...
// your e-stop switch to the Arduino reset pin, since it is the most correct way to do this.
  ...
  ISR(LIMIT_INT_vect) // DEFAULT: Limit pin change interrupt process.
  {
    ...
    // printPgmString(PSTR("int\n\r"));
    #ifdef SEEDER_CONFIG
    if (seederWorking){
      limits_disable();
      printPgmString(PSTR("s:1\r\n"));
      seederFinish = true;
      delay_ms(1000);
    }
    #endif
